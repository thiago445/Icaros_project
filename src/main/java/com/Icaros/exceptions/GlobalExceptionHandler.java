...
@Slf4j(topic = "GLOBAL_EXECPTION_HANDLER")
...
public class GlobalExceptionHandler extends ResponseEntityExceptionHandler implements AuthenticationFailureHandler {
	...
	@ResponseStatus(HttpStatus.NOT_FOUND)
	public ResponseEntity<Object> handleDataBindingViolationException(ObjectNotFoundException objectNotFoundException,
			WebRequest request) {
		log.error("Failed to save entity with associated data", objectNotFoundException);
		return buildErrorResponse(objectNotFoundException, HttpStatus.CONFLICT, request);
	}

	@ExceptionHandler(AuthenticationException.class)
	@ResponseStatus(HttpStatus.UNAUTHORIZED)
	public ResponseEntity<Object> handleAuthenticationExeption(AuthenticationException authenticationExeption,
			WebRequest request) {
		log.error("Authentication error", authenticationExeption);
		return buildErrorResponse(authenticationExeption, HttpStatus.UNAUTHORIZED, request);
	}
	...
	@ResponseStatus(HttpStatus.FORBIDDEN)
	public ResponseEntity<Object> handleAuthorizationExeption(AuthorizationExeption authorizationExeption,
			WebRequest request) {
		log.error("Authorization error", authorizationExeption);
		return buildErrorResponse(authorizationExeption, HttpStatus.FORBIDDEN, request);
	}
